...
@section('content')
    <div class="card">
                    ...
                    @foreach($sentences as $sentence)
                        <tr class="responsive-tr">
                            ...
                            <td class="table-item-max-width"
                                ...
                                @if(request('search'))
                                    {!! preg_replace('/('.preg_quote(request('search'), '/').')/i', '
                                    <mark>$1</mark>
                                    ', $sentence->sentence) !!}
                                @else
                                    {{$sentence->sentence}}
                            ...
                            <td>
                                <form action="{{route('sentences.destroy', $sentence->id)}}" method="post">
                                    ...
                                    <button type="submit" class="btn btn-danger waves-effect waves-light"
                                            onclick="return confirm('Вы действительно хотите безвозвратно удалить эти данные?');">
                                        Удалить
                                    </button>
                                </form>
                            </td>
        ...
        @else
            <div class="form-floating form-floating w-50" style="margin-right: 10px; padding: 10px" >
                <form method="POST" action="{{route('otherSentencesUpload')}}" enctype="multipart/form-data">
                    @csrf
                    <input class="form-control mb-6" type="file" id="formFile" name="file">
                    <button type="submit" class="btn btn-success">
                        Загрузить дополнительный корпус
                    </button>
                </form>
            </div>
        @endif
